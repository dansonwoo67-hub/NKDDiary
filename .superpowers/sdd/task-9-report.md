...
- The E2E runner safely loads `.env.local` without displaying values, chooses an empty local port, verifies a complete 200 NKD login page response, and tracks both Next and Playwright process trees from spawn through bounded graceful/forced shutdown.
...
3. Final runner-race cycle: RED added two tests that failed because signal-aware child state and body-inclusive readiness timeout helpers were absent. GREEN added them; focused helper tests pass 6/6.

## Verification commands and results

| Command | Result |
| --- | --- |
| `npm test -- scripts/playwright-e2e-config.test.ts` | PASS — 1 file, 6 tests. |
| `npx playwright test --list` | PASS — discovered smoke plus 3 serial lifecycle tests in Chromium and Pixel 7. |
| `npm test` | PASS — 39 files, 213 tests. |
| `npm run lint` | PASS. |
