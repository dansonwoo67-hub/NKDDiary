...
type EventRow = { id: string; name: string; icon: string; event_date: string; recurrence: CalendarRecurrence };
export type MemoryItem = { id: string; kind: "journal" | "mood" | "calendar"; title: string; occurredAt: string; href?: string };

export function buildMemoryFeed(input: { journals: JournalRow[]; moods: MoodRow[]; events: EventRow[]; today?: string }): MemoryItem[] {
  ...
  const events = input.events.flatMap((row) => {
    ...
    return occurredOn ? [{ id: row.id, kind: "calendar" as const, title: `${row.icon} ${row.name}`, occurredAt: `${occurredOn}T00:00:00+08:00`, href: "/calendar" }] : [];
  });
...
export async function listMemories(client: Pick<SupabaseClient, "from">, spaceId: string) {
  const [journals, moods, events] = await Promise.all([
    ...
    client.from("calendar_events").select("id, name, icon, event_date, recurrence").eq("space_id", spaceId),
  ]);
  if (journals.error || moods.error || events.error) throw new Error("Unable to load memories");
  return buildMemoryFeed({ journals: (journals.data ?? []) as JournalRow[], moods: (moods.data ?? []) as MoodRow[], events: (events.data ?? []) as EventRow[], today: getChinaDateString() });
}
