...
function validCompressedImage(value: FormDataEntryValue | null): value is File {
  return value instanceof Blob && value.type === "image/webp" && value.size > 0 && value.size <= MAX_IMAGE_BYTES;
}
...
export async function uploadJournalImageAction(
  ...
  if (!validCompressedImage(image)) return { ok: false, message: "图片格式或大小无效。" };
  ...
  const { error: uploadError } = await bucket.upload(imagePath, image, {
  ...
  try {
    const rpcResult = parsed.data.kind === "create-today"
      ? await client.rpc("create_today_diary", {
          p_space_id: spaceId,
      ...
      : await client.rpc("update_today_diary", {
          p_entry_id: entryId,
          p_title: parsed.data.title,
          p_content: parsed.data.content,
          p_image_path: imagePath,
    ...
    await bucket.remove([imagePath]).catch(() => undefined);
    return { ok: false, message: "操作失败，请稍后再试。" };
  }
}
