...
supabase/migrations/202607250001_mood_edit_window.sql
supabase/migrations/202607250002_independent_memories.sql
```
...
- The `memory-images` bucket exists, is **not public**, accepts only WebP up to
  800 KiB, and restricts reads to active members of the path's couple space.
- Mood and independent Memory mutations reject the partner, the guest, and the
  author at or after the 24-hour boundary.
...
Capture Home, Memories, Journal, Calendar, Settings, and login at all three
release viewports: `375 × 812` (`mobile-375`), `768 × 1024` (`tablet-768`), and
`1440 × 900` (`desktop-1440`). At every width confirm the five-item bottom
navigation remains visible, touch targets are at least 44×44 px, fixed
navigation does not cover the final content or modal actions, and the document
